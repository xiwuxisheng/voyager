...
##### WeexViewController -- 跨平台weex承载平台页
目前是放在项目中，关于这个属于单独的“第三方框架”，应该不继承BaseViewcontroller，而是单独作为一个组件独立存在，可以使用协议或者
路由的方式进行数据传递，后续应该考虑作为独立组件存在。

----

##### 日志类控制器应用
目前也是独立出去，不继承于该体系，原因是为了尽可能减少依赖，做到组件化之间的解藕，否则后续进行framework引入的时候依赖过多，导致独立的引入实际上引入全部，那么该组件化实质上是失败的组件设计。
